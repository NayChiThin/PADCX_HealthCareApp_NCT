...
object CloudFirestoreFirebaseApiImpl:FirebaseApi {
    ...
    override fun registerNewDoctor(
        ...
        db.collection(ROOT_DOCTORS)
            ...
            .addOnSuccessListener { Log.d("Firebase Success","Success register") }
            .addOnFailureListener { Log.d("Firebase Failure","Failure register") }
    }

    override fun getSpecialities(onSuccess: (specialities:List<SpecialityVO>) -> Unit, onFailure: (String) -> Unit) {
    ...
    override fun checkout(checkout: CheckoutVO, onSuccess: () -> Unit, onFailure: (String) -> Unit) {
    ...
    override fun getDoctorById(doctorId: String, onSuccess: (doctor: DoctorVO) -> Unit, onFailure: (String) -> Unit) {
        db.collection(ROOT_DOCTORS)
                .document(doctorId)
                .addSnapshotListener{value,error->
                    error?.let {
    ...
    override fun getCurrentPrescription(onSuccess: (prescription: List<PrescriptionVO>) -> Unit, onFailure: (String) -> Unit) {
        consultRef.collection(CHILD_PRESCRIPTION)
                .addSnapshotListener{value,error->
                    error?.let {
                        onFailure(it.message?:"Please check internet connection")
    ...
    override fun getCurrentCaseSummary(onSuccess: (questions: List<QuestionVO>) -> Unit, onFailure: (String) -> Unit) {
        consultRef.collection(CHILD_CASE_SUMMARY)
                .addSnapshotListener{value,error->
                    error?.let {
                        onFailure(it.message?:"Please check internet connection")
    ...
    override fun getCurrentPatientInfo(onSuccess: (patient: PatientVO) -> Unit, onFailure: (String) -> Unit) {
        val questionList : MutableList<QuestionVO> = arrayListOf()
        consultRef.collection(CHILD_CONSULTEES)
                .document("patient")
                        ...
                        rootRef
                            ...
                            .addSnapshotListener { value, error ->
                                error?.let {
                                    ...
                                    val data1 = value?.data
                                    data1?.let {
                                        patient = it.toPatientVO(recentDoctors,generalQuestions,addresses)
                                    }
                                }
                            }
                        // messages
